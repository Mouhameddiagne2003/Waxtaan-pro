...
interface AddGroupModalProps {
  onClose: () => void;
  onCreateGroup: (group: { name: string; members: { id: string; name: string }[] }) => void;
  contacts: { id: string; name: string }[];
  currentUser: { id: string; name: string };
}

const AddGroupModal: FC<AddGroupModalProps> = ({ onClose, onCreateGroup, contacts, currentUser }) => {
    ...
    try {
      ...
      onCreateGroup({
        ...
        members: group.members.map((id: string) => {
          ...
          const found = contacts.find(c => c.id === id) || (id === currentUser.id ? currentUser : { id, name: id });
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded shadow-lg p-6 w-full max-w-sm sm:max-w-md">
        ...
        <input
          type="text"
          className="w-full rounded border px-3 py-2 mb-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#25d366]"
          value={groupName}
        ...
        {feedback && <div className="text-xs text-center text-red-500 mb-2">{feedback}</div>}
        <div className="flex justify-end gap-2 mt-4">
          <button className="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300" onClick={onClose} disabled={loading}>Annuler</button>
          <button className="px-4 py-2 rounded bg-[#25d366] text-white font-semibold hover:bg-[#128C7E]" onClick={handleCreate} disabled={loading}>Créer</button>
        </div>
      </div>
    </div>
  );
